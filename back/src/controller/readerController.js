...
async function getReaders(req, res) {
  ...
  return readers.length > 0 ? res.status(200).json(readers) : res.status(204).send();
}
...
async function postReader(req, res) {
  ...
  try {
    const createdReader = await addReader(nome, cpf, email, telefone, formattedDate, new Date().toLocaleDateString('pt-BR'));
...
module.exports = { getReaders, getReaderBySearch, postReader, updateReader, deleteReader };
